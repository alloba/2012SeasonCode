...
public class CustomDrive
{
    ...
    public void driveNext()
    {
        ...
        //if no coordinate is found then reset the drive and do nothing
        if(c == null)
        {
            resetDrive();
            c = Navigation.getInstance().getNextCoordinate();
        }
    ...
    {
        Navigation.getInstance().reset();
        m_leftMotorPID.reset();
        m_rightMotorPID.reset();

        //re-enable the pid controllers because they are stopped
        //when reset
        m_leftMotorPID.enable();
        m_rightMotorPID.enable();
    }

    //runs periodically for drive
    public void run()
    {
        //if the robot reached its coordinate then reset the current position
        //to origin and pull the next coordinate
        if(onTarget())
        {
